...
const charts={};
const sensors=['level','tds'];
let staleNotified=false;
let lastState=null;
let blinkIntervals={};
sensors.forEach(id=>{
const ctx=document.getElementById(id+'Chart').getContext('2d');
charts[id]=new Chart(ctx,{type:'line',data:{labels:[],datasets:[{label:id.toUpperCase(),data:[],borderColor:'#0077ff',backgroundColor:'rgba(0,119,255,0.2)',tension:0.3,fill:true,pointRadius:0}]},options:{scales:{x:{display:false},y:id==="level"?{min:0,max:100}:{}}}});
});
function updateSystemState(state){
const badge=document.getElementById("systemState");
if(!state){badge.innerText="STATUS: OFFLINE";badge.style.background="gray";return;}
badge.innerText=`STATUS: ${state}`;
badge.style.background=state==="NORMAL"?"green":state==="WARNING"?"orange":state==="DANGER"?"red":state==="CRITICAL"?"darkred":"gray";
}
function startBlink(chart){
if(blinkIntervals[chart])return;
let on=false;
blinkIntervals[chart]=setInterval(()=>{
chart.data.datasets[0].backgroundColor=on?'rgba(255,0,0,0.25)':'rgba(255,0,0,0.05)';
chart.update();
on=!on;
},600);
}
...
sendBrowserNotification("Device Offline","No sensor data received. Check power or network.");
staleNotified=true;}
return;}
staleNotified=false;
sensors.forEach(id=>stopBlink(charts[id]));
document.getElementById('levelValue').innerText=data.level;
document.getElementById('percentValue').innerText=data.percentage;
document.getElementById('tdsValue').innerText=data.tds+' ppm';
updateSystemState(data.state);
if(data.state!==lastState){
if(data.state==="WARNING"){showNotification("⚠️ Water quality warning detected");}
if(data.state==="DANGER"){showNotification("🚨 Dangerous water quality detected!","danger",12000);sendBrowserNotification("Water Danger Alert","Water contamination detected");}
if(data.state==="CRITICAL"){showNotification("🆘 CRITICAL ALERT: Immediate action required!","danger",20000);sendBrowserNotification("CRITICAL WATER ALERT","Severe contamination suspected");}
lastState=data.state;}
const time=new Date().toLocaleTimeString();
sensors.forEach(id=>{
const chart=charts[id];
chart.data.labels.push(time);
if(id==="level"){chart.data.datasets[0].data.push(data.percentage);}else{chart.data.datasets[0].data.push(data.tds);}
if(chart.data.labels.length>10){chart.data.labels.shift();chart.data.datasets[0].data.shift();}
chart.update();
});
}catch(error){console.error("Fetch error:",error);}
}
setInterval(fetchData,1500);		const response = await fetch('/api/data');
		if (!response.ok)
		...
		if (data.stale) {
		 ...
		 sensors.forEach(id => startBlink(charts[id]));
			if (!staleNotified) {
				showNotification(
					"🔌 Device offline: No data received for 30 seconds",
					"danger",
					15000
				);
		...
		if (data.state !== lastState) {
			...
			if (data.state === "DANGER") {
				showNotification(
					"🚨 Dangerous water quality detected!",
					"danger",
					12000
				);
				sendBrowserNotification(
			...
			if (data.state === "CRITICAL") {
				...
				sendBrowserNotification(
					...
					"Severe contamination suspected"
				);
			}
			lastState = data.state;
		}
		...
		sensors.forEach(id => {
			...
			chart.data.labels.push(time);
			if (id === "level") {
				// Water level graph uses percentage
